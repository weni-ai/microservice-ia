...
# export GUNICORN_APP=${GUNICORN_APP:-".wsgi"}
...
if [[ "start" == "$1" ]]; then
    ...
    do_gosu "${PROJECT_USER}:${PROJECT_GROUP}" exec uvicorn app.main:main_app.api --host 0.0.0.0 --port 8000
    # do_gosu "${PROJECT_USER}:${PROJECT_GROUP}" exec gunicorn "${GUNICORN_APP}" -c "${GUNICORN_CONF}"   
