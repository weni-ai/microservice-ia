...
class TestProductsHandler(unittest.TestCase):
    ...
    def test_index(self):
        ...
        mock_product_index_request = ProductIndexRequest(
            catalog_id=catalog_id, product=mock_product
        )
        result = self.handler.index(mock_product_index_request)

        expected_product_index_response = ProductIndexResponse(
            catalog_id=catalog_id, documents=[mock_doc_id]
        )
    ...
    def test_batch_index(self):
        ...
        mock_ids = [
            "518c3834-e715-4f4d-9976-b4b318698e74",
            "4b4b9f6f-e177-4709-bd00-429f0675a0c4",
        ]

        self.mock_indexer.index_batch.return_value = mock_ids

        mock_batch_index_request = ProductBatchIndexRequest(
            catalog_id=catalog_id, products=mock_products
        )
        result = self.handler.batch_index(mock_batch_index_request)

        expected_batch_index_response = ProductIndexResponse(
            catalog_id=catalog_id, documents=mock_ids
        )
    ...
    def test_search(self):
        ...
        self.mock_indexer.search.assert_called_once_with(
            mock_request.search, mock_request.filter, mock_request.threshold
        )
    ...
    def test_delete(self):
        ...
        mock_request = ProductDeleteRequest(
            catalog_id=catalog_id, product_retailer_ids=product_retailer_ids
        )
        self.mock_indexer.delete.return_value = [mock_id]
        result = self.handler.delete(request=mock_request)
        self.assertEqual(
            result, ProductIndexResponse(catalog_id=catalog_id, documents=[mock_id])
        )
    ...
    def test_delete_batch(self):
        ...
        mock_ids = [
            "4d0c1730-133d-4b49-b407-5141bd571cca",
            "9d063d4b-20f7-4fcd-9751-247c97357efb",
        ]
        mock_request = ProductDeleteRequest(
            catalog_id=catalog_id, product_retailer_ids=product_retailer_ids
        )
        ...
        self.assertEqual(
            result, ProductIndexResponse(catalog_id=catalog_id, documents=mock_ids)
        )
