...
class ProductsIndexer(IDocumentIndexer):
    ...
    def index(self, catalog_id: str, product: Product):
        results = self._search_products_by_retailer_id(catalog_id, [product.product_retailer_id])
        ids = []
        if len(results) > 0:
    ...
    def index_batch(self, catalog_id: str, products: list[Product]):
        ...
        if len(results) > 0:
            ids = [item["_id"] for item in results]
            self.storage.delete(ids=ids)
        docs = [Document(page_content=product.title, metadata=product) for product in products]
    ...
    def search(self, search, filter=None, threshold=0.1) -> list[Product]:
        ...
        products = [
            Product.from_metadata(doc.metadata)
    ...
    def delete(self, catalog_id, product_retailer_id):
        results = self._search_products_by_retailer_id(catalog_id, [product_retailer_id])
        ids = []
        if len(results) > 0:
            ids = [item["_id"] for item in results]
            self.storage.delete(ids=ids)
